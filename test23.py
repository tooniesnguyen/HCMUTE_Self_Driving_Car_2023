...
def move_right(img):
    ...
    avg_area_down = np.mean(img[30:60,140:160])
    ...
    if Move_Right:
        angle = 17
    if avg_area_down <= 200 and Move_Right:
...
def move_left(img):
    ...
    avg_area_down = np.mean(img[30:60,:20])
    
    # line_decided = np.mean(img[:,159])

    if avg_area_down > 25 and avg_area_top <= 10:
        Move_Left = 1
    if Move_Left:
        print("Goooooooooooooooooooooooooooooooooooooo")
        angle = -17
    if avg_area_down <= 200 and Move_Left:
        print("Stoppppppppppppppppppppppppppppppppppppppppp")
        Move_Left = 0
        Start_Turn_Left = 0
        ...
        while True:
            ...
            for result in results:
                try: 
                    ...
                    if name_img == 2 and Move_Left == 0 and Move_Right == 0:

                    if area >= 2000 and not (Ready_Mode_Right or Ready_Mode_Left):
            ...
            speed = PID(err_speed, 25, 1.5, 0.01, mode = "speed")

            ###########################################################################################

            Kp,Ki,Kd = 0.9, 1.7, 0.01
            arr = []
            lineRowAB = img[50:80,:]
            lineRowBL = img[50,:]
            ...
            if (Ready_Mode_Right or Ready_Mode_Left) and name_img is None:
                No_Detect += 1
                if No_Detect >= 25 and Ready_Mode_Right:
                    Start_Turn_Right = 1
                    No_Detect = 0
                    Ready_Mode_Right = 0
                elif No_Detect >= 25 and Ready_Mode_Left:
                    Start_Turn_Left = 1
                    No_Detect = 0
                    Ready_Mode_Left = 0
            if Start_Turn_Right:
            ...
            ################################ PID Angle ######################################
            if not (Move_Right or Move_Left):
